     ...
     <form method="post" action="../member/join_ok.do" @submit="submitForm">
      <table class="table">
        ...
        <tr>
         ...
         <td width=90% class="inline">
          <input type=text name=addr2 id=addr2 size=55 class="input-sm" v-model="addr2">
         </td>
        </tr>
  	...
  	new Vue({
  		...
  		methods:{
  			submitForm:function(e){
  				if(this.id && this.name && this.pwd && this.pwd1 && this.sex && this.email && this.birthday && this.post
  						&& this.addr1 && this.phone1 && this.phone && !this.idOk && !this.emailOk){
  					return true;
  				}
  			...
  			idCheckValidate:function(id){
  				...
  				if(id_res.length<6 || id_res.length>12){
  					this.idOk='아이디는 6자리에서 12자리 이내로 입력해 주세요'
  					return;
  				}
  			...
  			pwdValidate:function(){
  				...
  				if(pwd.length < 8 || pwd.length>20){
  					this.pwdOk='비밀번호를 8자리에서 20자 이내로 입력하세요'
  					return;
  				}
