...
<!-- 
	...
	DI (의존성 주입) : 스프링에서 동작을 위해 필요한 데이터를 전송
					...
					= method DI => 객체 생성시(init-method), 객체 소멸시(destory-method)
	...
	View(JSP) : request, session전송을 받아서 화면 출력
		...
		클래스 등록 (모든 클래스가 등록되는 것은 아니다) => 인터페이스, VO제외
		XML
			<bean class="" id="">
		...
		어노테이션
		 	...
		 	*** 입사 (개발자)
		 		1) 소스분석 (회사)
		 			1. web.xml / 2. server.xml / 3. application.xml 순서로 먼저 분석
		 			클래스 (Model => DAO (Service) => JSP)
		 	*** React/Vue/MSA(Spring Cloud)
		 	------------------------------- CI/CD : 지속적인 통합(도커), 지속적인 배포(젠킨스)
