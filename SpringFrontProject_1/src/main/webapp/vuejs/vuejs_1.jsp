...
<%-- 
	...
	2. 디렉티브 
		...
		=> 양방향 통신 / 단방향 통신 (Vue/React)
		   -------- AngularJS에서 파생 => GO(제임스 고슬링 개발=>자바랑 비슷) / C# / Java
	3. 컴포넌트 : 재사용
		Vue.component('modal',
		template:'<html>...')
	4. 데이터를 읽어서 출력 (클라이언트 <=> 서버통신) => axios
		...
		=> 일반 문자열 / JSON 으로 결과값 받기 가능 => @RestController
		=> id나 class를 사용하지 않는다 ref=""
	5. 라우터 (화면 변경) => Spring에서 화면변경
		=> 검색, 로그인 처리, 댓글
		
		가상돔 예시
		<html>														html
			<head>										head					body
				<link>								link	script			div		div
				<script></script>							 DOM : Document Object Model
			</head>												=>트리형태로 저장
			<body>
				<div></div>
				<div></div>
	...
	6. Vue생성
		...
		=> <img :src="poster">
		=> : => <input type="text" :value="id">  ===> :은 v-bind
														 ------- :으로 대신 사용할 수 있어서 생략이 가능
 --%>
 <div class="container">
 	<div class="row">
 	...
 	<!-- v-model => 멤버변수값을 변경 -->
 	<div id="print"> <!-- 출력 : msgㅇ 저장된 내용을 출력 -->
 	{{msg}}
 	</div>
 	</div>
 </div>
