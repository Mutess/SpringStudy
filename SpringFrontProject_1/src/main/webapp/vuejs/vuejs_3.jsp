...
<div class="container">
	<h1 class="text-center">VueJS(디렉티브:v-for(제어문))</h1>
 	...
 	<table class="table">
 		...
 		<tbody>
 			<tr v-for="vo in movie"> <!-- 대괄호(vo) 하나씩 가지고 와라 -->
...
<script>
	new Vue({
		...
		created:function(){
			console.log("Created:Vue객체를 생성완료...")
		...
		mounted:function(){
			console.log("Mounted:가상메모리(가상돔)에 HTML을 올리기 완료...")
			// $(function())m window.onload
			// 가상돔 / 실제 돔을 비교해서 변경 => diff
			// 가장 많이 사용되는 상태 (서버에서 데이터를 읽어오는 경우)
		},
		beforeUpdate:function(){
			console.log("beforeUpdate:상태가 변경(데이터값이 변경)되기 직전상태...")
			// 멤벼 변수 설정 : react=> state, Vue => data => data를 다른 메소드에서 변경시에는 this..
		},
		updated:function(){
			console.log("updated:데이터변경이나 수정이 완료된 상태 => mounted를 재호출...")
		},
		beforeDestroy:function(){
			console.log("beforeDestroy:메모리가 해제되기 전의 상태...")
		},
		destroy:function(){
			console.log("destroy:화면이 변경 => Vue객체 해제...")
		},
		//사용자 정의 함수
		methods:{
			find:function(){
