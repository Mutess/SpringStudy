	...
	------ 
	  DI : Spring을 통해서 객체생성 ~ 객체 소멸
	  					------
	  					  ...
	  					  2. 생성자 => 생성자 DI
	  					  3. setter르 이용하는 방법 => setter DI
	  					  ----------------------------------
	  					  XML 이용 , 자바를 이용
	  AOP : 프로그램 (핵삼 코드, 공통 코드)	
	  						------ getConnection(), disConnection()
	  		=> 클래스마다 공통으로 사용되는 메소드를 모았다가 필요한 기시에 자동 호출이 가능
	  		=> 트랜잭션, LOG, 보안
	  MVC : 웹 제작 (View/Model)
	  		Model : 데이터 관리 (자바)
	  		View : 데이터 출력 전송
	  		...
	  		-------------------------------------- 이미 라이브러리로 제작
	  		 			DispatcherServlet => 톰캣에 의해 구동
	  		 								 -----------
	  		 								 	web.xml
	  		 			1) DispatherServlet등록
	  		 			2) DispatherServlet을 찾는 방법
	  		 				<url-pattern>*.do</url-pattern>
	  		 				<url-pattern>/</url-pattern>
	  		 				=> PathValiable admin/hong/1234)
	  		*** 요청을 받는 경우 (요청 데이터 전송)
	  			=> request에 담겨서 들어온다 => request.getParameter();
	  				** request는 사용자 정보 (IP,PORT) 가지고 있음 (즉, 보안상의 문제가 생김)
	  					=> 가급적 사용하지 않음
	  					=> 사용자 정의 메소드를 만들때 매개변수를 설정하면 DispatcherServlet에 의해 매개변수가 채워짐
	  					=> JSP로 결과값을 전송 => Model
	  	Model을 작성
	  	1) @Controller 설정해야 메모리 할당이 가능하다(메소드를 찾는 경우)
	  	2) 메소드
	  		= 리턴형 : String, void
	  				 ------ 화면이동에 관련된 JSP정보/리다이렉트 정보
	  		= 매개변수 : JSP에서 제공하는 내장 객체 설정
	  				   ...
	  				   *** 순서는 상관없다
	  	3) 찾기 (메소드)
		  	@GetMapping => Getmapping => Get방식으로 요청 (default) => <a> ajax, axios...
		  	@PostMapping => <form> , ajax, axios
		  	 							   ----- ajax대체 => Vue, React
		  	 				axios.get(), axios.post()
		  	---------------------------- Spring 4.3이후부터 나눠져 있다.
		  	@RequestMapping => GET/POST르 동시에 처리가 가능
