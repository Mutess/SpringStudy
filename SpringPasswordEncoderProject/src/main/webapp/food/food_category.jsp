...
<div class="container-fluid">
	...
	<div class="row">
		...
		<div class="col-md-4" v-for="vo in cate_list">
			      <div class="thumbnail">
<!-- 			        <a :href="'../food/food_list.do?fno='+vo.fno"> 여기서 링크처리하면 Controller에서 처리 -->
		...
		<div class="col-sm-5" v-show="inShow">
			<div class="jumbotron">
				<h3 class="text-center">{{cate_info.title}}</h3>
				<h4 class="text-center">{{cate_info.subtitle}}</h4>
			</div>
...
new Vue({
	...
	// 생성자(mounted) 초기화 할때 사용
	mounted:function(){
		this.categoryGetData(1);
	},
	methods:{
		categoryGetData:function(cno){
			axios.get('../food/food_category_vue.do',{ //여기서 링크처리하면 RestController에서 처리
				params:{
					cno:cno //앞에는 키값
				}
			}).then(response=>{
		...
		foodGetList:function(cno,bool){
			...
			}).then(response=>{ //매개변수로 값을 받아옴
				console.log(response.data)
				this.cate_info=response.data
			}).catch(error=>{
				console.log(error.response) //error안에 있는 다른 response
			})
