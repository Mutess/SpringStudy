...
<script type="text/javascript"
	src="//dapi.kakao.com/v2/maps/sdk.js?appkey=b5203b97e62633c04cb8e55d2e14616d&libraries=services"></script>
<link rel="stylesheet"
...
<body>
		...
		<div class="row">
			<div class="col-sm-7">
				...
				<table class="table">
					...
					<tr v-if="food_detail.parking!=null">
					...
					<tr v-if="food_detail.menu!='no'">
						...
						<td width="80%">
							<ul>
								<!-- <li v-for="menu in food_detail.menu.split('원')">{{menu}}원</li> -->
							...
							<table class="table" v-for="re in reply_data">
								<tr>
									...
									<td class="text-right"><span
										v-if="re.id==food_detail.sessionId"> <input
								...
								<tr style="display: none;" :id="'reply'+re.no" class="updates">
									<td colspan="2"><textarea rows="3" cols="55"
											style="float: left;" ref="msg" id="msg"<%-- ref는 제어용 v-model은 data:{여기 안에 들어감} --%>>{{re.msg}}</textarea>
										<input type="button" value="댓글수정"
										style="height: 65px; background-color: blue; color: white;" @click="replyUpdate(re.no)">
									</td>
								</tr>
							</table>
						</td>
				...
				<table class="table" v-if="food_detail.sessionId!=null">
					<tr>
						<td><textarea rows="3" cols="55" style="float: left;"
								ref="msg"
								<%-- ref는 제어용 v-model은 data:{여기 안에 들어감} --%> v-model="msg"></textarea>
							<input type="button" value="댓글쓰기"
							style="height: 65px; background-color: blue; color: white;"
							@click="replyWrite()"></td>
					</tr>
				</table>
			</div>
	...
	new Vue({
		...
		data:{
			...
			menu:[],
			no:0
		}, // 여기에 있는 변수의 데이터값이 바뀌면 아래에서 변경가능
		mounted:function(){
			// 1. 상세정보 읽기
...
//				this.food_detail.menu=this.food_detail.menu.split('원').filter(menu => menu !== '');
		...
		methods:{
			...
			addScript:function(){
				...
				script.src='http://dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=b5203b97e62633c04cb8e55d2e14616d&libraries=services'
			...
			replyUpdate:function(no){
				...
				axios.post('../food/reply_update_vue.do',null,{
					...
					console.log(response.data)
					this.reply_data=response.data // updated => 변경된 데이터값으로 변경 후 출력
					$('#reply'+no).hide();
					$('#up'+no).val("수정");
					this.no=0;
				}).catch(error=>{
