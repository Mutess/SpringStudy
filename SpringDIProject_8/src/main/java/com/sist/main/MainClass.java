...
/*
	1.프레임워크
		...
		= 프레임워크 : 레고 => 변경가능
			...
			=> 스프링의 목표(로드 존슨) => Open Source
				1) 클래스와 클래스간의 느슨한 결합, 낮은 결합도 유지
								 ---------------------
								 	변경할때 추가할때 다른 클래스에 영향이 없이 개발
								 	POJO (독립적인 클래스 제작)
				2) 메소드마다 반복해서 사용하는 소스코드 => 공통 모듈로 분리 (응집력이 높은 프로그램 제작)
					=> AOP : OOP를 보완
			=> 클래스를 모아서 관리 (생성 ~ 소멸 : 생명주기를 관리) => 컨테이너
		...
		1. 컨테이너의 종류
			BeanFactory : 가장 단순한 컨테이너
				|		  사용자가 만든 클래스만 관리 => DI로 값을 넘겨줌
				|		  -------------------------------------- CORE
				|
			ApplicationContext : AOP, 메세지 지원 (Java Message Service)
				|				 => 구현된 클래스
		...
		*** 스프링에서 지원하는 라이브러리
			Spring Core : DI (객체의 생성 ~ 소멸:생명주기) =>  Container
		...
		*** XML사용법
			...
			= 클래스명(메모리 할당이 가능)을 전송
			  ----------
			  XML
			  Annotation 		메타정보 =======> Spring Container에 의해 메모리 할당
			  자바코드
			  ----------
			  XML 이용
			  	=> 클래스 한개를 설정
			  		...
			  		-------------------------
			  			b			new B()
			  		-------------------------
			  		*** 프로그램 동작 시작 => 클래스를 모아서 저장 : 주소값이 변경이 안됨
			  			=> 싱글톤
			  	=> 패키지 단위 설정
...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		ApplicationContext app=new ClassPathAppliationContext(path);
